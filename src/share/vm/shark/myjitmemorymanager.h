#ifndef MYJITMEMORYMANAGER_H
#define MYJITMEMORYMANAGER_H
...
namespace {
  ...
  class MyJITMemoryManager;

  class JITSlabAllocator : public SlabAllocator {
    MyJITMemoryManager &JMM;
  public:
    JITSlabAllocator(MyJITMemoryManager &jmm) : JMM(jmm) { }
    virtual ~JITSlabAllocator() { }
    virtual MemSlab *Allocate(size_t Size);
    virtual void Deallocate(MemSlab *Slab);
  };

  /// MyJITMemoryManager - Manage memory for the JIT code generation.
  /// This splits a large block of MAP_NORESERVE'd memory into two
  /// sections, one for function stubs, one for the functions themselves.  We
  /// have to do this because we may need to emit a function stub while in the
  /// middle of emitting a function, and we don't know how large the function we
  /// are emitting is.
  class MyJITMemoryManager : public JITMemoryManager {
    ...
    sys::MemoryBlock allocateNewMemoryRWX(size_t arg1);
  public:
    MyJITMemoryManager();
    ~MyJITMemoryManager();
...
MyJITMemoryManager::MyJITMemoryManager()
  :
#ifdef NDEBUG
    PoisonMemory(false),
#else
...
void MyJITMemoryManager::AllocateGOT() {
...
sys::MemoryBlock MyJITMemoryManager::allocateNewSlab(size_t size) {
  // Allocate a new block close to the last one.
  std::string ErrMsg;
  sys::MemoryBlock *LastSlabPtr = LastSlab.base() ? &LastSlab : 0;
  //sys::MemoryBlock B = sys::Memory::AllocateRWX(size, LastSlabPtr, &ErrMsg);
  sys::MemoryBlock B = allocateNewMemoryRWX(size);
  if (B.base() == 0) {
...
/// implementation details of MyJITMemoryManager.
bool MyJITMemoryManager::CheckInvariants(std::string &ErrorStr) {
...
JITMemoryManager *JITMemoryManager::CreateDefaultMemManager() {
  std::cout << "JITMemoryManager is defined by myself" << std::endl;
  return new MyJITMemoryManager();
}

// Allocate memory for code in 512K slabs.
const size_t MyJITMemoryManager::DefaultCodeSlabSize = 512 * 1024;

// Allocate globals and stubs in slabs of 64K.  (probably 16 pages)
const size_t MyJITMemoryManager::DefaultSlabSize = 64 * 1024;

// Waste at most 16K at the end of each bump slab.  (probably 4 pages)
const size_t MyJITMemoryManager::DefaultSizeThreshold = 16 * 1024;


#endif // MYJITMEMORYMANAGER_H
